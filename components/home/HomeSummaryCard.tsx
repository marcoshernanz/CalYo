import calcRatio from "@/lib/utils/calcRatio";
import { SharedValue, useDerivedValue } from "react-native-reanimated";
import Button from "../ui/Button";
import Card from "../ui/Card";
import { StyleSheet, View } from "react-native";
import getColor from "@/lib/ui/getColor";
import Text from "../ui/Text";
import CircularProgress from "../ui/CircularProgress";
import { ComponentType } from "react";
import { LucideProps } from "lucide-react-native";

type MacroCardProps = {
  item: {
    name: string;
    value: number;
    target: number;
    Icon: ComponentType<LucideProps>;
    color: string;
  };
  progress: SharedValue<number>;
};

export default function HomeSummaryCard({ item, progress }: MacroCardProps) {
  const progressMacro = useDerivedValue(
    () => calcRatio(item.value, item.target) * progress.value
  );

  return (
    <Button variant="base" size="base" style={{ flex: 1 }}>
      <Card style={styles.summaryCard}>
        <Text size="12" weight="600" color={getColor("mutedForeground")}>
          {item.name}
        </Text>
        <View style={styles.summaryCardValueContainer}>
          <Text size="18" weight="600">
            {item.value}
          </Text>
          <Text
            size="10"
            color={getColor("mutedForeground")}
            style={styles.summaryCardTargetText}
          >
            {" "}
            / {item.target}
          </Text>
        </View>
        <View style={styles.summaryCardProgressContainer}>
          <CircularProgress
            progress={progressMacro}
            color={item.color}
            strokeWidth={4}
            size={80}
          />
          <View style={styles.summaryCardIconContainer}>
            <item.Icon size={18} strokeWidth={2.25} />
          </View>
        </View>
      </Card>
    </Button>
  );
}

const styles = StyleSheet.create({
  summaryCard: {
    backgroundColor: getColor("background"),
    flex: 1,
    padding: 16,
  },
  summaryCardValueContainer: {
    flexDirection: "row",
    alignItems: "flex-end",
    paddingBottom: 12,
    paddingTop: 4,
  },
  summaryCardTargetText: {
    paddingBottom: 3,
  },
  summaryCardProgressContainer: {
    height: 80,
    width: 80,
    alignItems: "center",
    justifyContent: "center",
    alignSelf: "center",
  },
  summaryCardIconContainer: {
    position: "absolute",
    alignItems: "center",
    justifyContent: "center",
    width: 32,
    height: 32,
    borderRadius: 999,
    backgroundColor: getColor("muted"),
  },
});
